...
class Bee(object):
    ...
    def __init__(self, lower, upper, fun,instance, funcon=None):
        """
        ...
        self.value = fun(self.vector, self.instance)
    ...
    def _random(self, lower, upper):
        ...
        # Randomly arrange the whole numbers in the range lower to upper(inclusive) in a list. and the numbers should not be repeated.
        
        self.vector = list(range(lower[0], upper[0]+1))  # Create a list of numbers in the specified range.
        random.shuffle(self.vector)  # Randomly shuffle the list.
...
class BeeHive(object):
    ...
    def run(self):
        ...
        # print("Best for each iteration",cost["best"])
        return cost
    ...
    def __init__(self                 ,
        ...
        # creates a bee hive
        self.population = [ Bee(lower, upper, fun=self.evaluate,instance=self.instance) for i in range(self.size) ]
    ...
    def send_employee(self, index):
        ...
        d = random.randint(0, self.upper[0]-1)
        ...
        #TODO FIX THIS
        # zombee.vector[d] = self._mutate(d, index, bee_ix) # ORIGINAL
        zombee.vector = self._mutate(d, index, bee_ix) # NEW

        # checks boundaries
        zombee.vector = self._check(zombee.vector, dim=d)

        # computes fitness of mutant
        zombee.value = self.evaluate(zombee.vector, self.instance)
        zombee._fitness()
    ...
    def send_onlookers(self):
        ...
        while (numb_onlookers < self.size):
            ...
            # self.probas list contains the probabilities associated to the employed bees Calculated using the formula of 0.9 * v / max_values + 0.1 (Karaboga classic ABC implementation)
            
            # beta is the roulette wheel parameter, the 
            beta += phi * max(self.probas) 
            beta %= max(self.probas) #Wrappping the beta value to the range of probabilities
    ...
    def send_scout(self):
        ...
        if (trials[index] > self.max_trials):
            ...
            self.population[index] = Bee(self.lower, self.upper, self.evaluate,instance=self.instance)
    ...
    def _mutate(self, dim, current_bee, other_bee):
        ...
        """
        # ORIGINAL is below
        # return int(self.population[current_bee].vector[dim]    + \
        #        (random.random() - 0.5) * 2                 * \
        #        (self.population[current_bee].vector[dim] - self.population[other_bee].vector[dim]))
        
        # TODO FIX THIS
        # NEW strategy is below
        # Create a copy of the current bee's vector
        mutated_vector = list(self.population[current_bee].vector)

        # Randomly choose two distinct indices for swapping
        index1, index2 = random.sample(range(self.dim), 2)

        # Swap the values at the chosen indices
        mutated_vector[index1], mutated_vector[index2] = mutated_vector[index2], mutated_vector[index1]
